...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Couldn't sign you in. Try again.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function handleGoogleCredential(idToken) {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Couldn't sign in with Google.");
    }
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 bg-neutral-50 dark:bg-neutral-950">
      <div className="w-full max-w-sm">
        ...
        <p className="text-sm text-neutral-500 mb-6">Sign in to keep writing.</p>
        ...
        <p className="text-sm text-neutral-500 text-center mt-6">
          New to Flawr?{" "}
          <Link to="/signup" className="text-flawr-800 dark:text-flawr-200 font-medium">
            Create an account
          </Link>
        </p>
      </div>
